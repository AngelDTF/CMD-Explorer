...
if exist "%tempdir%\config.cmd" call "%tempdir%\config.cmd"
if not exist "%tempdir%\config.cmd" goto :Personalize
goto :Introductory
...
echo [CMDE Version B1.2]
echo Welcome to CMDE, %pname%!
echo.
echo About:
...
echo New Personalization Update:
echo New username choice
echo New color choices
echo.
echo WARNING:
echo If you have purchased this program somewhere, DEMAND YOUR MONEY BACK IMMEDIATELY! CMDE is a free software and should be downloaded from TheonTutorials' GitHub.
echo ________________________________________________________________________________________________________________________
pause
goto :Start
...
echo We are here to guide you through the setup of CMDE!
echo First we want you to choose your username.
echo What would you like us to call you?
set /p pname=Name: 
echo %pname% huh? Strange, but I guess it would work.
echo Anyways %pname%, we just got 1 more thing for you to do.
echo %pname%, please enter what you would like for your Text/Background colors.
echo Enter the background first, then the text color. For example, you would write 0F for a black background and white text.
echo.
echo [0] = Black	[8] = Gray
...
color %pcol%
echo Please wait...
echo REM These files should not be touched! If you need to reset your personal settings, type '/resetpersonal' at the hard drive select screen!>"%tempdir%\config.cmd"
echo set pname=%pname%>>"%tempdir%\config.cmd"
echo set pcol=%pcol%>>"%tempdir%\config.cmd"
echo Perfect! We are all set up now!
echo Now you will be able to use CMDE!
echo If you ever need to modify these settings,
echo type '/resetpersonal' at the hard drive select screen.
pause
goto :Introductory
...
cls
if "%v%"=="6.1" echo WARNING Compression (.zip only) only works on Windows 8 and above!
echo Choose A Drive:
echo.
for /f "skip=1 delims=" %%x in ('wmic logicaldisk get caption') do @echo.%%x
echo.
echo Or type '/eject' to eject the disc tray.
echo Or type '/resetpersonal' to reset your personal settings.
echo ________________________________________________________________________________________________________________________
set /p ds=Drive Selection: 
if "%ds%"=="/eject" goto ejecttray
if "%ds%"=="/resetpersonal" goto :resetpersonal
...
:CantCompress
title CMDE Incompatible Version
echo We are so sorry!
echo Your version of Windows, '6.1' doesn't fit the requirements for regular compressing (.zip)!
pause
goto :ZipManagement
